...
func run(ctx context.Context, args []string, out, errOut io.Writer) int {
	...
	whoami := fs.Bool("whoami", false, "Show the token's account and role")
	...
	n := 0
	for _, v := range []bool{*devices, *software, *tests, *types, *whoami} {
		if v {
			n++
		}
	}
	if n != 1 {
		return fail(fmt.Errorf("select exactly one of --devices, --software, --tests, --types, --whoami"))
	}
	...
	if !*tests && (*forDevice != "" || *forSoftware != "" || *outcome != "" || *tag != "") {
		return fail(fmt.Errorf("test filters require --tests"))
	}
	if *version != "" && !(*software && *get != "" || *tests && *forSoftware != "") {
		return fail(fmt.Errorf("--version requires --software --get or --tests --for-software"))
	}
	listFilters := *search != "" || len(filter) > 0 || *sort != "" || *order != "" || *limit != 0 || *typeKey != "" || *status != "" || *latest || *forDevice != "" || *forSoftware != "" || *outcome != "" || *tag != ""
	...
	if (*types || *whoami) && (listFilters || *fields) {
		return fail(fmt.Errorf("list filters and --fields require --devices, --software, or --tests"))
	}
	if *fields && (*search != "" || len(filter) > 0 || *sort != "" || *order != "" || *limit != 0 || *status != "" || *latest || *forDevice != "" || *forSoftware != "" || *outcome != "" || *tag != "") {
	...
	for key, value := range map[string]string{"search": *search, "device_type": *typeKey, "status": *status, "outcome": *outcome, "tag": *tag, "sort": *sort, "order": *order} {
		if value != "" {
			if _, ok := q[key]; ok {
	...
	for key, allowed := range map[string]string{"status": "available,checked_out,inventory,missing,broken", "outcome": "pass,fail,warn", "tag": "adhoc,acceptance,end-to-end,automated"} {
		if key == "status" && !*devices || key != "status" && !*tests {
			continue
		}
