...
def manifest(x_testbench_plugin: str | None = Header(None), x_testbench_plugin_secret: str | None = Header(None)):
    ...
    return {"configuration_defaults": {"http_port": int(os.getenv("TB_INFO_HTTP_PORT", "80")), "https_port": int(os.getenv("TB_INFO_HTTPS_PORT", "443"))}, "id": PLUGIN_ID, "label": "Device Info Agent", "version": "1.0.0", "protocol_version": 1,
      "recommended_fields": [
...
def _discovery_roles_to_request(roles: dict) -> list[str]:
...
def _submit_result(marker: str, entity: dict, roles: dict, provider: str, model: str, selection: str, run_id: str):
...
def _create_and_watch(run_id: str, entity: dict):
    ...
    try:
        ...
        urls = _device_urls(address, entity.get("_plugin_configuration"))
        url = urls[0]
        ...
        recipe_contract = (
            ...
            "The steps array must list, in chronological order, only successful browser steps you actually performed to reach the hardware, firmware, and MAC values. "
        ...
        roles = entity.get("_plugin_roles", {})
        discovery_roles_to_request = _discovery_roles_to_request(roles)
        ...
        prompt += "\nDo not look for or return MAC roles omitted from that list; those fields already have inventory values."
        prompt += "\n" + contract
        ...
        ai_secret = os.getenv("TB_INFO_AI_SECRET_NAME", "testbench-device-info-ai")
        provider = "openaiCompatible"
        base_url = os.getenv("TB_INFO_AI_URL", "")
        discovery_model = os.getenv("TB_INFO_AI_MODEL", "")
        repeat_model = os.getenv("TB_INFO_AI_REPEAT_MODEL", "") or discovery_model
        selection = "saved_recipe" if recipe else "discovery"
        ...
        provider_env = [
            ...
            client.V1EnvVar(name="OPENAI_API_KEY", value_from=client.V1EnvVarSource(secret_key_ref=client.V1SecretKeySelector(
                name=ai_secret,
                key=os.getenv("TB_INFO_AI_SECRET_KEY", "OPENAI_API_KEY"),
            ))),
        ...
        while time.monotonic() < deadline:
            ...
            if pod_name:
                try:
                    ...
                    if marker:
                        try: _submit_result(marker, entity, roles, provider, model, selection, run_id)
        ...
        _submit_result(marker, entity, roles, provider, model, selection, run_id)
...
def invoke(action_id: str, body: dict, x_testbench_plugin: str | None = Header(None), x_testbench_plugin_secret: str | None = Header(None)):
    _auth(x_testbench_plugin, x_testbench_plugin_secret)
    if action_id != "device-info-agent.research": raise HTTPException(404, "Unknown action")
    entities = body.get("entities", [])
    if len(entities) != 1: raise HTTPException(422, "Research requires one device")
...
def cancel(run_id: str, x_testbench_plugin: str | None = Header(None), x_testbench_plugin_secret: str | None = Header(None)):
    ...
    if run.get("state") in {"starting", "running"}:
        ...
        try: client.BatchV1Api().delete_namespaced_job(f"info-{run_id}", NAMESPACE, propagation_policy="Background")
